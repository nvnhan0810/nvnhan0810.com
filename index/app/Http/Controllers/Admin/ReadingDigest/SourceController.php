...
use App\Jobs\ReadingDigest\FetchSourceJob;
...
class SourceController extends Controller
{
    ...
    {
        $sources = RdSource::query()->orderBy('name')->get();
    ...
    {
        $source = RdSource::query()->with('tagMappings.taxonomyNode')->findOrFail($id);
    ...
    {
        $source = RdSource::query()->findOrFail($id);
    ...
    {
        RdSource::query()->findOrFail($id)->delete();
    ...
    {
        $source = RdSource::query()->findOrFail($id);
        $fetcher = app(SourceFetcherRegistry::class)->for($source);
     ...
     */
    private function formProps(?RdSource $source): array
    {
        return [
            'source' => $source,
            'sourceTypes' => collect(SourceType::cases())->map(fn ($t) => [
